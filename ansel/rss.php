...
if (empty($rss)) {
    ...
    case 'all':
        try {
            $images = $storage->getRecentImages();
    ...
    case 'gallery':
        ...
        if ($gallery->hasPermission($GLOBALS['registry']->getAuth(), Horde_Perms::SHOW) &&
            ...
            if (!$gallery->countImages() && $gallery->hasSubGalleries()) {
                $subgalleries = $storage->listGalleries(array('parent' => $gallery));
                $subs = array();
                foreach ($subgalleries as $subgallery) {
                    $subs[] = $subgallery->id;
                }
                $images = $storage->getRecentImages($subs);
            } else {
        ...
        if (!count($images)) {
            ...
            $viewurl = Ansel::getUrlFor('view',
                                        array('view' => 'Gallery',
                                              'gallery' => $id),
                                        true);
            $img = $storage->getImage($gallery->getKeyImage(Ansel::getStyleDefinition('ansel_default')));
    ...
    case 'user':
        $galleries = array();
        try {
            $shares = $storage->listGalleries(array('attributes' => $id));
        ...
        if (isset($galleries) && count($galleries)) {
            try {
                $images = $storage->getRecentImages($galleries);
            } catch (Ansel_Exception $e) {
                 Horde::log($e->getMessage(), 'ERR');
            }
            if (count($images)) {
    ...
    case 'tag':
        ...
        try {
            $images = $storage->getImages(array('ids' => $images['images']));
        } catch (Ansel_Exception $e) {
             Horde::log($e->getMessage(), 'ERR');
             $images = array();
        }
        ...
        for ($i = 0; $i < $cnt; ++$i) {
            ...
            if (empty($galleries[$gallery_id])) {
                try {
                    $galleries[$gallery_id]['gallery'] = $storage->getGallery($gallery_id);
                } catch (Ansel_Exception $e) {}
            }
